    ...
    for n in range(1, max_n + 1):  # Principal quantum number (n >= 1)
        for l in range(0, n):  # Azimuthal quantum number (0 <= l < n)
            for m in range(-l, l + 1):  # Magnetic quantum number (-l <= m <= l)
                psi=plot(n, l, m)
                np.save(f'PLane_Waves/analytical_psis/Psi_{n}_{l}_{m}', psi, allow_pickle=True)
                print(f'Calculating: {n=},{l=},{m=}')
...
def psi_plot(n, l, m, grid):
    ...
    res=16
    x = np.linspace(-grid, grid, res+1)
    y = np.linspace(-grid, grid, res+1)
    z = np.linspace(-grid, grid, res+1)
